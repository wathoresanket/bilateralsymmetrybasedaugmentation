...
def data_augmentation_original():
    ...
    mkdirs("dentex_dataset/segmentation/enumeration32_bilateral_symmetry_based_augmentation/masks")
    mkdirs("dentex_dataset/segmentation/enumeration32_bilateral_symmetry_based_augmentation/xrays")
    ...
    for image_info in dataset_json["images"]:
        ...
        mask.save(f"dentex_dataset/segmentation/enumeration32_bilateral_symmetry_based_augmentation/masks/{image_info['file_name']}")
        shutil.copy(
            f"dentex_dataset/origin/quadrant_enumeration/xrays/{image_info['file_name']}",
            f"dentex_dataset/segmentation/enumeration32_bilateral_symmetry_based_augmentation/xrays/{image_info['file_name']}",
        )

    save_json("dentex_dataset/segmentation/enumeration32_bilateral_symmetry_based_augmentation/image_names.json", image_names)
...
def data_augmentation_flip():
    ...
    for image_name in os.listdir(xray_dir):
        ...
        for y in range(height):
            for x in range(width):
                # If the grayscale value of the pixel is between 9-16 2
                if 9 <= new_mask[y, x] <= 16:
                    new_mask[y, x] -= 8
                # If the grayscale value of the pixel is between 17-24 3
                elif 17 <= new_mask[y, x] <= 24:
                    new_mask[y, x] += 8
                # If the grayscale value of the pixel is between 1-8 1
                elif 1 <= new_mask[y, x] <= 8:
                    new_mask[y, x] += 8
                # If the grayscale value of the pixel is between 25-32 4
...
if __name__ == "__main__":
    ...
    # Output directory
    output_dir = 'dentex_dataset/segmentation/enumeration32_bilateral_symmetry_based_augmentation/'
